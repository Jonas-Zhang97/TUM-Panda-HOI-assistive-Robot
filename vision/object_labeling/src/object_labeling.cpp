...
{
  object_point_cloud_sub_ = nh.subscribe<sensor_msgs::PointCloud2>(objects_cloud_topic_, 1, &ObjectLabeling::cloudCallback, this);
  object_detections_sub_ = nh.subscribe<darknet_ros_msgs::BoundingBoxes>("/darknet_ros/bounding_boxes", 1, &ObjectLabeling::detectionCallback, this);
  camera_info_sub_ = nh.subscribe<sensor_msgs::CameraInfo>(camera_info_topic_, 1, &ObjectLabeling::cameraInfoCallback, this);

  labeled_object_cloud_pub_ = nh.advertise<PointCloudl>("/labeled_objects", 1);
  text_marker_pub_ = nh.advertise<visualization_msgs::MarkerArray>("/visualization_marker_array",1);
...
{
  // Extract the point cloud into clusters
  ...
  // Transform centroids into camera frame: camera_depth_optical_frame
  ...
  for (size_t i = 0; i < centroids.size(); ++i)
  {
    ...
    ROS_INFO_STREAM("Centroid position in camera optical frame = " << centroid_camera);
  }
  ...
  for (size_t i = 0; i < centroids_camera.size(); ++i)
  {
    ...
    ROS_INFO_STREAM("Centroid position in pixel coordinates = " << pixel_coord_2d);
  }
  ...
  for(size_t i = 0; i < detections_.size(); ++i)
  {
    ...
    ROS_INFO_STREAM("centroid of bounding box = " << box_center);
